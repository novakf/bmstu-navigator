...
class EditorStartup {
  ...
  async init() {
    ...
    try {
      ...
      const newSeEditPrefsDialog = document.createElement(
    ...
    const liElems = this.$svgEditor.querySelectorAll(
      "button, select, input:not(#text)"
    )
    const self = this
    ...
    window.addEventListener("resize", () => {
      Object.entries(winWh).forEach(([type, val]) => {
        ...
        this.workarea["scroll" + (type === "width" ? "Left" : "Top")] -=
          (curval - val) / 2
        winWh[type] = curval
      })
    })
      ...
      function () {
        ...
        if (this.configObj.curConfig.baseUnit) {
          $editDialog.setAttribute(
            "baseunit",
            this.configObj.curConfig.baseUnit
          )
        }
        ...
        if (this.configObj.curConfig.gridColor) {
          $editDialog.setAttribute(
            "gridcolor",
            this.configObj.curConfig.gridColor
          )
        }
  ...
  async extAndLocaleFunc() {
    ...
    try {
        ...
        this.configObj.curConfig.extensions.map(async (extname) => {
          ...
          try {
            ...
            const imported = await import(
              `${extPath}/${encodeURIComponent(extname)}/${encodeURIComponent(
                extname
              )}.js`
            )
      ...
      await Promise.all(
        this.configObj.curConfig.userExtensions.map(
          async ({ pathName, config }) => {
            ...
            try {
              ...
              return this.addExtension(
                name,
                initfn && initfn.bind(this, config),
                {}
              )
            } catch (err) {
              // Todo: Add config to alert any errors
