...
export default {
  ...
  async init(_S) {
     ...
     */
    const importImage = (e) => {
      $id("se-prompt-dialog").title = this.i18next.t(
        "notification.loadingImage"
      )
      $id("se-prompt-dialog").setAttribute("close", false)
      e.stopPropagation()
      e.preventDefault()
      const file =
        e.type === "drop" ? e.dataTransfer.files[0] : e.currentTarget.files[0]
      if (!file) {
      ...
      if (file.type.includes("svg")) {
        ...
        reader.onloadend = (ev) => {
          ...
          const newElement = this.svgCanvas.importSvgString(
            ev.target.result,
            imgImport.shiftKey
          )
    ...
    const clickClear = async function () {
      ...
      const ok = await seConfirm(
        svgEditor.i18next.t("notification.QwantToClear")
      )
      if (ok === "Cancel") {
        return
      }
    ...
    const clickOpen = async function () {
      ...
      try {
        ...
        // CHANGE
        svgEditor.onOpenDocument && svgEditor.onOpenDocument()
