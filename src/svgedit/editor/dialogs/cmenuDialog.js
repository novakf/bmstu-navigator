...
export class SeCMenuDialog extends HTMLElement {
  ...
  constructor() {
    ...
    this.$pasteInPlaceLink =
      this._shadowRoot.querySelector('#se-paste-in-place');
  ...
  init(i18next) {
    ...
    this.setAttribute(
      ...
      i18next.t('tools.paste_in_place')
    );
  ...
  connectedCallback() {
    ...
    svgEditor.$click(this.$pasteLink, (evt) =>
      onMenuClickHandler(evt, 'paste')
    );
    svgEditor.$click(this.$pasteInPlaceLink, (evt) =>
      onMenuClickHandler(evt, 'paste_in_place')
    );
    svgEditor.$click(this.$deleteLink, (evt) =>
      onMenuClickHandler(evt, 'delete')
    );
    svgEditor.$click(this.$groupLink, (evt) =>
      onMenuClickHandler(evt, 'group')
    );
    svgEditor.$click(this.$ungroupLink, (evt) =>
      onMenuClickHandler(evt, 'ungroup')
    );
    svgEditor.$click(this.$moveFrontLink, (evt) =>
      onMenuClickHandler(evt, 'move_front')
    );
    svgEditor.$click(this.$moveUpLink, (evt) =>
      onMenuClickHandler(evt, 'move_up')
    );
    svgEditor.$click(this.$moveDownLink, (evt) =>
      onMenuClickHandler(evt, 'move_down')
    );
    svgEditor.$click(this.$moveBackLink, (evt) =>
      onMenuClickHandler(evt, 'move_back')
    );
  }
}
